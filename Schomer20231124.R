...
srcdir <- "/Users/kurtfristrup/Schomer/"
...
rm(list=c("bright", "widfor", "hermit"))
...
modls2 <- read.csv(paste0(srcdir,"/output2/HermitModSel.csv"))
tmp <- sapply(modls2[-1,2][cumsum(modls2$weight[-1]) < 0.999], function(md)
  paste0("Pleasant1 ~ ",md))
tmp <- sub(patt="\\+ Heli1 ", repl=" ", tmp)
tmp <- sub(patt="Heli1:+", repl="", tmp)
tmp <- sub(patt=":+Heli1", repl="", tmp)
modls3 <- formulasUnique(tmp)
modls3 <- c(modls3,
            sub(pattern="~",replacement="~ QtP +",modls3),
            sub(pattern="~",replacement="~ Totalseg +",modls3),
            sub(pattern="~",replacement="~ cHeli:Totalseg +",modls3),
            sub(pattern="~",replacement="~ cHeli:QtP +",modls3))
nms3 <- c("Pleasant1",varList(termList(modls3)))
summary(hermerg[,nms3,with=F]) #checking the data, found all heli==1
ixHerMerg <- !apply(is.na(hermerg[,match(nms3,names(hermerg)),with=F]),1,any)
hermerg <- hermerg[ixHerMerg,..nms3]
print(nbrOfWorkers())
parReps <- seq(along=modls3)
handlers("progress", "beepr")
tstrt <- Sys.time()
with_progress({
  p <- progressor(along = parReps)
  qmods <- future_lapply(parReps, function(x, ...) {
    tst2 <- clmm(formula = modls3[x], data=hermerg)
    p(sprintf("x=%g, tdiff=%g", x, Sys.time()-tstrt))
    return(tst2)
  })
})
print(Sys.time() - tstrt)
Qresults <- modSumOut(qmods, "Pleasant1")
write.csv(file=paste0(srcdir,"output/Qmodels.csv"), Qresults$ModlSumm)
write.csv(file=paste0(srcdir,"output/Qvariables.csv"), Qresults$CoefSumm)
# summed weight for models containing QtP
...
######### 15.5 hours, 5.5 Gb for one 2000 tree forest
######### #future_lapply doesn't seem to know about data.table
######### check whether Hwt is present
...
write.csv(file = paste0(srcdir,"Output/","VariableImportance1000.csv"),
          cbind(pleasVimp, pleasDmeans[vimpNames], pleasSigns[vimpNames],
                ...
                heliSigns[vimpNames]))
# why is cumLaeq1 a good predictor?
array(as.numeric(grall[, summary(na.omit(cumLaeq1)), keyby=Trail]$V1), dim=c(6,3),
	dimnames=list(names(summary(na.omit(grall$cumLaeq1))), levels(grall$Trail)))
grall[, mean(as.ITime(Date)), by=c("Trail", "Jet1")]
